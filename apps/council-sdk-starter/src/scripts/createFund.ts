import {
  CappedFrozenLockingVault,
  CouncilContext,
  FrozenLockingVaultFactory,
  VotingContract,
} from "@council/sdk";
...
export async function createFund(): Promise<void> {
  ...
  const cappedFrozenLockingVault = new CappedFrozenLockingVault(
    addresses.cappedFrozenLockingVault,
    context,
  );
  const frozenLockingVaultFactory = new FrozenLockingVaultFactory(
    addresses.frozenLockingVaultFactory,
    context,
  );
  ...
  const tx = await coreVoting.createFund(
    signer,
    vaults,
    targets,
    calldatas,
