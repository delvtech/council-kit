...
export const { command, aliases, describe, builder, handler } =
  createCommandModule({
    command: "vesting-vault [OPTIONS]",
    aliases: ["VestingVault"],
    describe: "Deploy a VestingVault contract",
    ...
    handler: async (args) => {
      ...
      const { address } = await deployVestingVault({
        token,
        staleBlockLag: lag,
        account,
        rpcUrl,
      ...
      signale.success(`VestingVault deployed @ ${address}`);
    },
  });
